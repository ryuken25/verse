...
const steps = [
  ...
  { id: 'mix', kicker: 'Mix', title: 'Users can add randomness too', body: 'A fair design can combine server randomness with user randomness, wallet address, block data, or nonce.', bullets: ['Server seed', 'User seed or nonce', 'Block data'] },
  { id: 'reveal', kicker: 'Reveal', title: 'The secret is revealed after the result', body: 'After the game or reward draw, the original seed is revealed. Anyone can hash it again and check that it matches.', bullets: ['Reveal original seed', 'Recompute hash', 'Compare with commit'] },
  { id: 'verify', kicker: 'Verify', title: 'Anyone can recompute the outcome', body: 'If users run the same formula with the same inputs, they should get the same result. That is independent verification.', bullets: ['No hidden server trust', 'Repeatable result', 'Public verification'] },
];

const badges = [
  { id: 'commit', label: 'Commit', icon: <Dice5 className="w-5 h-5 text-purple-300" />, ring: 0, angle: 0, duration: 22 },
  { id: 'mix', label: 'Mix', icon: <Shuffle className="w-5 h-5 text-blue-300" />, ring: 0, angle: 120, duration: 24 },
  { id: 'reveal', label: 'Reveal', icon: <Eye className="w-5 h-5 text-green-300" />, ring: 1, angle: 60, duration: 26 },
  { id: 'verify', label: 'Verify', icon: <CheckCircle className="w-5 h-5 text-cyan-300" />, ring: 1, angle: 240, duration: 28 },
];
