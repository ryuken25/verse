...
import { X, Wallet, ExternalLink, Copy, Check, LogOut, AlertCircle, Smartphone, Monitor, Award, Flame, BookOpen, Trophy } from 'lucide-react';
...
const TOTAL_LESSONS = 48; // Adjust based on actual course data
...
export default function WalletModal({ isOpen, onClose }: WalletModalProps) {
  ...
  useEffect(() => {
    return onProgressUpdated((payload) => {
      if (payload.address === (address || 'guest')) {
        setProgress(payload.newProgress);
      }
    });
  }, [address]);
        ...
        {view === 'profile' && address && progress && (
          <div className="space-y-4">
            ...
            <div className={`p-4 rounded-xl ${isPolygonChain(chain?.id) ? 'bg-green-500/10 border border-green-500/20' : 'bg-yellow-500/10 border border-yellow-500/20'}`}>
              <div className="flex items-center justify-between">
                ...
                {!isPolygonChain(chain?.id) && (
                  <button type="button" onClick={() => switchChain({ chainId: polygon.id })} disabled={isSwitching}
            ...
            <div className="p-4 rounded-xl bg-gradient-to-br from-purple-500/10 to-blue-500/10 border border-purple-500/20">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-white font-bold text-lg">
                    {isMax ? 'Level MAX' : `Level ${level}`}
                  </span>
                  {isMax && <span className="text-xs px-2 py-0.5 bg-yellow-500/20 text-yellow-400 rounded-full">VERSE Academy Master</span>}
                </div>
              ...
              {isMax && <p className="text-green-400 text-xs text-center mt-1">All courses completed!</p>}
            </div>
